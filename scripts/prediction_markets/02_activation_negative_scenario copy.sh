...
ACTIVATION_FEE_SMALL=100000000000 # 1000 KONG, token has decimal precision 8
...
# Step 1.5: Have Carol try to place a bet while the market is still pending
section "Step 1.5: Having Carol try to place a bet on a pending market (should fail)" 
...
# Check if market is still not active
run_dfx "dfx canister call prediction_markets_backend get_market \"(\$MARKET_ID)\"" "Checking market status remains in Pending state"


# Step 2: Alice places activation bet with a too small activation fee amount (1000 KONG) on "Yes"
section "Step 2: Alice placing activation bet with a too small activation fee amount (1000 KONG) on 'Yes'"
run_dfx "dfx identity use alice" "Switching to Alice identity"
EXPIRES_AT=$(echo "$(date +%s)*1000000000 + 60000000000" | bc)  # approval expires 60 seconds from now
...
echo -e "${CMD_COLOR}Approving tokens for transfer...${RESET}"
run_dfx "dfx canister call \${KONG_LEDGER} icrc2_approve \"(record {
	amount = \$(echo \"\${ACTIVATION_FEE_SMALL} + \${KONG_FEE}\" | bc);
	expires_at = opt \${EXPIRES_AT};
	spender = record {
		owner = principal \\\"\${PREDICTION_MARKETS_CANISTER}\\\";
	};
...
run_dfx "dfx canister call prediction_markets_backend place_bet \"(\$MARKET_ID : nat, 0 : nat, \$ACTIVATION_FEE_SMALL : nat, opt \\\"\${KONG_LEDGER}\\\")\"" "Placing bet"
...
run_dfx "dfx canister call prediction_markets_backend get_market \"(\$MARKET_ID)\"" "Checking market status is Active"
