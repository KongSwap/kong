...
# Test script for creating and interacting with a prediction market using ksICP tokens
...
KSICP_LEDGER=$(dfx canister id ksicp_ledger)
KSICP_FEE=$(dfx canister call ${KSICP_LEDGER} icrc1_fee "()" | awk -F'[:]+' '{print $1}' | awk '{gsub(/\(/, ""); print}')
KSICP_FEE=${KSICP_FEE//_/}

echo "=== Testing ksICP Multi-Token Market ==="
echo "Prediction Markets Canister: ${PREDICTION_MARKETS_CANISTER}"
echo "ksICP Ledger: ${KSICP_LEDGER}"
echo "ksICP Fee: ${KSICP_FEE}"

# Step 1: Create a market using ksICP token
echo "Creating a new market with ksICP tokens..."
...
# Step 2: Place a bet using ksICP
echo "Placing a bet with ksICP tokens..."

# The activation threshold for ksICP is 25 ICP
BET_AMOUNT=2500000000 # 25 ksICP (8 decimals)
...
echo "Approving ksICP tokens for transfer..."
dfx canister call ${KSICP_LEDGER} icrc2_approve "(record {
	amount = $(echo "${BET_AMOUNT} + ${KSICP_FEE}" | bc);
...
echo "Placing bet of ${BET_AMOUNT} ksICP on outcome ${OUTCOME_INDEX} for market ${MARKET_ID}..."
...
SECOND_BET_AMOUNT=2500000000 # 25 ksICP
SECOND_OUTCOME_INDEX=1 # Betting on "No"

# Approve the tokens for transfer
dfx canister call ${KSICP_LEDGER} icrc2_approve "(record {
	amount = $(echo "${SECOND_BET_AMOUNT} + ${KSICP_FEE}" | bc);
	expires_at = opt ${EXPIRES_AT};
	spender = record {
		owner = principal \"${PREDICTION_MARKETS_CANISTER}\";
	};
...
echo "=== ksICP Market Test Complete ==="
