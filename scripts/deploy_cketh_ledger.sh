...
if [ "$1" == "staging" ]; then
	...
	if CANISTER_ID=$(jq -r ".[\"cketh_ledger\"][\"local\"]" "${root_dir}"/canister_ids.all.json); then
		[ "${CANISTER_ID}" != "null" ] && {
			echo "Deploying cketh_ledger with ID: ${CANISTER_ID}"
...
TOKEN_NAME="Chain-key Ethereum (KongSwap Test Token)"
TOKEN_DECIMALS=18
