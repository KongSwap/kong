...
TOKEN_NAME="Ethereum (KongSwap Test Token)"
TOKEN_DECIMALS=18
TRANSFER_FEE=10_000
...
if [ "$1" == "staging" ]; then
	...
	if [ -f "${PROJECT_ROOT}/.env" ]; then
		. "${PROJECT_ROOT}/.env"
		if [ ! -z "${CANISTER_ID_CKETH_LEDGER}" ]; then
			SPECIFIED_ID="--specified-id ${CANISTER_ID_CKETH_LEDGER}"
		else
			echo "Warning: CANISTER_ID_CKETH_LEDGER not found in .env"
		fi
	else
