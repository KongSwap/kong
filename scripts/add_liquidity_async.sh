...
# ckUSDT
CKUSDT_TOKEN="ckUSDT"
CKUSDT_LEDGER=$(dfx canister id ${NETWORK} $(echo ${CKUSDT_TOKEN} | tr '[:upper:]' '[:lower:]')_ledger)
...
# ICP/ckUSDT pool
ICP_LEDGER=$(dfx canister id ${NETWORK} $(echo ${ICP_TOKEN} | tr '[:upper:]' '[:lower:]')_ledger)
ICP_DECIMALS=$(dfx canister call ${ICP_LEDGER} icrc1_decimals '()' | awk -F"[^0-9]*" '{print $2}')
...
# ckUSDC/ckUSDT pool
CKUSDC_LEDGER=$(dfx canister id ${NETWORK} $(echo ${CKUSDC_TOKEN} | tr '[:upper:]' '[:lower:]')_ledger)
CKUSDC_DECIMALS=$(dfx canister call ${CKUSDC_LEDGER} icrc1_decimals '()' | awk -F"[^0-9]*" '{print $2}')
...
CKUSDC_FEE=$(dfx canister call ${NETWORK} ${CKUSDC_LEDGER} icrc1_fee "()" | awk -F'[:]+' '{print $1}' | awk '{gsub(/\(/, ""); print}')
...
# ckBTC/ckUSDT pool
CKBTC_LEDGER=$(dfx canister id ${NETWORK} $(echo ${CKBTC_TOKEN} | tr '[:upper:]' '[:lower:]')_ledger)
CKBTC_DECIMALS=$(dfx canister call ${CKBTC_LEDGER} icrc1_decimals '()' | awk -F"[^0-9]*" '{print $2}')
...
CKBTC_FEE=$(dfx canister call ${NETWORK} ${CKBTC_LEDGER} icrc1_fee "()" | awk -F'[:]+' '{print $1}' | awk '{gsub(/\(/, ""); print}')
...
# ckETH/ckUSDT pool
CKETH_LEDGER=$(dfx canister id ${NETWORK} $(echo ${CKETH_TOKEN} | tr '[:upper:]' '[:lower:]')_ledger)
CKETH_DECIMALS=$(dfx canister call ${CKETH_LEDGER} icrc1_decimals '()' | awk -F"[^0-9]*" '{print $2}')
...
CKETH_FEE=$(dfx canister call ${NETWORK} ${CKETH_LEDGER} icrc1_fee "()" | awk -F'[:]+' '{print $1}' | awk '{gsub(/\(/, ""); print}')
