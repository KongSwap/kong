...
if [ "$1" == "staging" ]; then
	...
	if CANISTER_ID=$(jq -r ".[\"ckbtc_ledger\"][\"local\"]" "${root_dir}"/canister_ids.all.json); then
		[ "${CANISTER_ID}" != "null" ] && {
			echo "Deploying ckbtc_ledger with ID: ${CANISTER_ID}"
...
TOKEN_NAME="Chain-key Bitcoin (KongSwap Test Token)"
TOKEN_DECIMALS=8
TRANSFER_FEE=10
