...
pub fn get_unresolved_transactions() -> Vec<(u64, FailedTransaction)> {
    STABLE_FAILED_TRANSACTIONS.with(|txs| {
        let txs = txs.borrow();
        txs.iter().filter(|(_, tx)| !tx.resolved).map(|(id, tx)| (id, tx.clone())).collect()
    })
}
...
pub fn get_all_transactions() -> Vec<(u64, FailedTransaction)> {
    STABLE_FAILED_TRANSACTIONS.with(|txs| {
        let txs = txs.borrow();
        txs.iter().map(|(id, tx)| (id, tx.clone())).collect()
    })
}
        ...
        Some(tx) if !tx.resolved => {
            ...
            match transfer_token(tx.recipient, tx.amount.clone(), &tx.token_id, None).await {
                Ok(block_index) => {
                    ...
                    ic_cdk::println!("Successfully retried transaction {}: block_index {}", tx_id, block_index);
                    Ok(Some(block_index))
                }
                Err(e) => {
