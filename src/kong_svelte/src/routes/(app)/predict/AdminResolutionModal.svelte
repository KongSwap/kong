...
>
  <div class="flex flex-col gap-4">
    ...
    <div class="text-kong-text-secondary text-sm">
      Resolving a market is <span class="font-bold text-kong-accent-red">irreversible</span> and will distribute winnings to users. This action cannot be undone.
    </div>
    <div class="text-kong-text-primary text-base font-medium">
      {market?.question}
    </div>
    <div class="bg-kong-bg-light border border-kong-border rounded p-3 text-kong-text-secondary text-sm">
        ...
        <button
          class="w-full p-2 rounded border transition-colors text-left
            {selectedOutcome === BigInt(index)
              ? 'border-kong-accent-green bg-kong-accent-green/10 font-semibold text-kong-accent-green'
              : 'border-kong-border hover:border-kong-accent-green hover:bg-kong-accent-green/5'}"
    ...
    <div class="flex gap-3 justify-end pt-2">
      <button
        class="px-4 py-2 rounded bg-kong-bg-light text-kong-text-primary hover:bg-kong-bg-dark/40 transition-colors"
        on:click={close}
      ...
      <button
        class="px-4 py-2 rounded bg-kong-accent-green text-kong-text-on-primary font-bold hover:bg-kong-accent-green/90 disabled:opacity-50 transition-colors"
