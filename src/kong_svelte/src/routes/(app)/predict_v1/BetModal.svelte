<script lang="ts">
  ...
  function calculatePotentialWin(
    ...
    const betAmountScaled = toFixed(betAmount.toString(), 8);
    ...
    const yourShareOfPool = outcomePool === 0 ? 1 : Number(betAmountScaled) / (Number(outcomePool) + Number(betAmountScaled));
