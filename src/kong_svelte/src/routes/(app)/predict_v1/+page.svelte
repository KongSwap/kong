<script lang="ts">
  ...
  import { placeBet, getAllBets, isAdmin } from "$lib/api/predictionMarketLegacy";
  ...
  import {
    ...
    type SortOption,
    type StatusFilter,
  } from "$lib/stores/marketStoreLegacy";
      ...
      <div class="flex gap-3 w-full justify-center md:justify-end">
        {#if $auth.isConnected}
          <ButtonV2
            ...
            variant="solid"
            size="md"
            onclick={() => goto("/predict_v1/history")}
          >
            Prediction History
          </ButtonV2>
        {/if}
              ...
              <MarketSection
                markets={$marketStore.statusFilter === "resolved"
                  ? $filteredMarkets.resolved.filter(
                      (marketResult) => "Closed" in marketResult.market.status,
                    )
                  : $marketStore.statusFilter === "voided"
                    ? $filteredMarkets.resolved.filter(
                        (marketResult) => "Voided" in marketResult.market.status,
                      )
                    : $marketStore.statusFilter === "expired"
                      ...
                      : $marketStore.statusFilter === "all"
                        ? [
                            ...
                            ...($filteredMarkets.expired_unresolved || []),
                            ...($filteredMarkets.resolved.map(r => r.market) || []),
                          ]
                        : $filteredMarkets.active}
                {openBetModal}
