<script lang="ts">
  ...
  async function loadPage(pageNumber: number, forPrincipal: string) {
    ...
    try {
      ...
      console.error(
        `Failed to load swap transactions for principal: ${forPrincipal}`,
        error,
      );
      transactionStore.set([]);
  ...
  const swapTransactions = derived(transactionStore, ($transactions) => {
    return $transactions.filter(
      (tx) => tx && tx.tx_type === "swap" && tx.details,
    );
  });
...
<Panel variant="transparent">
  ...
  <div>
    ...
    {:else}
      ...
      <div
        class="hidden sm:grid sm:grid-cols-[1fr,1fr,1fr,0.8fr] gap-4 px-4 py-2 text-sm text-kong-text-secondary font-medium border-b border-kong-bg-primary"
      >
        <div>From</div>
        <div>To</div>
      ...
      <div class="divide-y divide-kong-bg-primary">
        {#each $swapTransactions as tx (`${tx.tx_id}-${tx.timestamp}`)}
          ...
          <div
            class="hidden sm:grid sm:grid-cols-[1fr,1fr,1fr,0.8fr] sm:gap-4 sm:items-center px-4 py-3 hover:bg-kong-bg-primary/30 transition-colors"
          >
            <!-- From -->
            <div class="flex items-center gap-2">
              <TokenImages
                tokens={[findToken(tx.details.pay_token_canister)].filter(
                  Boolean,
                )}
                size={28}
              />
              <div class="text-sm">
                <div class="font-medium text-kong-error">
                  -{formatToNonZeroDecimal(tx.details.pay_amount)}
                  {tx.details.pay_token_symbol}
                </div>
              </div>
            </div>
            ...
            <div class="flex items-center gap-2">
              <TokenImages
                tokens={[findToken(tx.details.receive_token_canister)].filter(
                  Boolean,
                )}
                size={28}
              />
              <div class="text-sm">
                <div class="font-medium text-kong-success">
                  +{formatToNonZeroDecimal(tx.details.receive_amount)}
            ...
            <div class="text-right text-sm">
              <div class="font-medium">
                ${formatToNonZeroDecimal(
                  tx.details.receive_amount *
                    Number(
                      findToken(tx.details.receive_token_canister)?.metrics
                        ?.price || 0,
                    ),
                )}
              </div>
            ...
            <div class="text-right text-sm text-kong-text-secondary">
              ...
              {new Date(Number(tx.timestamp) / 1_000_000).toLocaleTimeString(
                [],
          ...
          <div
            class="sm:hidden p-4 hover:bg-kong-bg-primary/30 transition-colors"
          >
            ...
            <div class="flex justify-between items-center mb-3">
              ...
              <div class="flex items-center gap-1">
                ...
                <div class="flex items-center">
                  <TokenImages
                    tokens={[
                      findToken(tx.details.receive_token_canister),
                    ].filter(Boolean)}
                    size={24}
                  />
                </div>
    ...
    <div
      class="mt-4 sm:mt-6 pt-3 border-t border-kong-bg-primary flex justify-between sm:justify-end"
    >
      <div
        class="flex items-center gap-1 sm:gap-2 py-2 text-xs sm:text-sm text-kong-text-secondary"
      >
        <span class="whitespace-nowrap"
          >Page {$currentPage} of {Math.max($totalPages, 1)}</span
        >
