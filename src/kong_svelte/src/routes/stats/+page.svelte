...
<script lang="ts">
  ...
  import { poolStore, poolsList, poolsTotals, poolsLoading, poolsError } from "$lib/stores/poolStore";
...
<main class="flex min-h-[95vh] bg-sky-100 relative">
  <div class="w-1/12 font-alumni z-[2]">
    <StatsSignPost
      totalTvl={$poolsTotals.tvl}
      totalVolume={$poolsTotals.volume24h}
      totalFees={$poolsTotals.fees24h}
    />
  </div>

  <div class="flex-grow z-10 w-10/12 flex pt-32">
    <div class="bg-k-light-blue bg-opacity-70 border-[5px] border-black p-0.5 w-full max-w-5xl mx-auto">
      <div class="inner-border p-4 w-full h-full">
        ...
        <div class="overflow-x-scroll">
          {#if $poolsLoading}
            <div class="text-center p-4">Loading pools...</div>
          {:else if $poolsError}
          ...
          {:else}
            <table class="w-full text-black font-alumni">
              ...
              <!-- Table Body -->
              <tbody>
                {#if $filteredSortedPools.length === 0}
                    ...
                    <tr class="border-b-2 border-black text-xl md:text-3xl">
                      ...
                      <td class="uppercase font-bold cursor-pointer" on:click={() => goto(`/stats/pools?id=${pool.id}`)}>
                        <div class="flex items-center">
                          <div class="isolate flex -space-x-3 overflow-hidden p-2 w-[5rem]">
                            <img 
                              class="relative z-30 inline-block h-9 w-9 rounded-full ring-2 ring-sky-500 bg-white" 
                              src={$tokenStore.tokens.find(token => token.canister_id === pool.address_0)?.logo || "/tokens/not_verified.webp"} 
                              alt={pool.symbol_0}
                            />
                            <img 
                              class="relative z-20 inline-block h-9 w-9 rounded-full ring-2 ring-sky-500 bg-white" 
                              src={$tokenStore.tokens.find(token => token.canister_id === pool.address_1)?.logo || "/tokens/not_verified.webp"} 
                              alt={pool.symbol_1}
                            />
                          </div>
                      ...
                      <td class="p-2 text-right">${formatNumberCustom(pool.price.toString(), 2)}</td>
                      <td class="p-2 text-right">${formatNumberCustom(pool.tvl.toString(), 2)}</td>
                      <td class="p-2 text-right">${formatNumberCustom(pool.rolling_24h_volume.toString(), 2)}</td>
                      <td class="p-2 text-right">{formatNumberCustom(pool.rolling_24h_apy.toString(), 2)}%</td>
