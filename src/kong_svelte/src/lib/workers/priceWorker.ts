...
class PriceWorkerImpl implements PriceWorkerApi {
  ...
  async startUpdates(): Promise<void> {
    console.log('Price worker starting updates');
    ...
    try {
        ...
        const batchUpdates = await Promise.all(
          batch.map(async (token) => {
            try {
              ...
              if (currentPrice > 0) {
                // Get the previous price before updating lastKnownPrices
                const previousPrice = this.lastKnownPrices[token.canister_id] || currentPrice;
                
                // Update lastKnownPrices after capturing the previous price
      ...
      const validUpdates = allUpdates.filter(update => {
        const isValid = update && 
                       update.price > 0 && 
                       Math.abs(update.price - update.previousPrice) > 0.000001;
        
        if (isValid) {
          console.log(`Valid price update for ${update.id}:`, {
            current: update.price,
            previous: update.previousPrice,
            diff: update.price - update.previousPrice
          });
        }
        return isValid;
      });
      ...
      if (validUpdates.length > 0) {
        console.log('Sending price updates:', validUpdates);
        self.postMessage({ type: 'price_update', updates: validUpdates });
      } else {
        console.log('No significant price changes to report');
      }
    } catch (error) {
  ...
  private schedulePriceUpdate(): void {
    ...
    console.log(`Scheduling price updates every ${interval}ms`);
    
    this.updateInterval = self.setInterval(() => {
      if (!this.isPaused) {
        this.postPriceUpdate().catch(err => 
