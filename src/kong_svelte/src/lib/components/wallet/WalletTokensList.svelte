<script lang="ts">
  ...
  import {
    discoverTokens,
    shouldRefreshTokenCache,
  } from "$lib/services/tokenDiscoveryService";
  import {
    analyzeUserTokens,
    applyTokenSync,
  } from "$lib/services/tokenSyncService";
  import {
    loadUserBalances,
    setLastRefreshed,
        ...
        (() => {
          ...
          const icpProcessed = Array.from(currentEnabled)
            .map((canisterId) => {
              ...
              if (!token) {
                console.warn(
                  `[WalletTokensList] Enabled token ${canisterId} not found in tokenData map during derived recalc.`,
                );
          ...
          const sorted = combined.sort((a, b) => {
            const aUsdValue = Number(
              $currentUserBalancesStore[a.address]?.in_usd || "0",
            );
            const bUsdValue = Number(
              $currentUserBalancesStore[b.address]?.in_usd || "0",
            );
  ...
  async function runTokenDiscovery() {
    ...
    try {
      ...
      if (
        !shouldRefreshTokenCache(
          allAvailableTokensCacheTimestamp,
          allAvailableTokensCache,
        )
      ) {
      ...
      const currentEnabledTokens = $userTokens.tokens.filter(
        (token) => token.address && enabledTokenIds.has(token.address),
      );
      ...
      if (
        discoveryResult.tokensToAdd.length > 0 ||
        discoveryResult.tokensToRemove.length > 0
      ) {
  ...
  async function handleSyncTokens(skipInitialRefresh = false) {
    ...
    try {
      ...
      if (
        result.syncStatus &&
        (result.syncStatus.added > 0 || result.syncStatus.removed > 0)
      ) {
        showSyncConfirmModal = true;
      } else {
  ...
  function handleClickOutside(event: MouseEvent) {
    if (
      selectedTokenElement &&
      !selectedTokenElement.contains(event.target as Node)
    ) {
      closeDropdown();
    }
  }
    ...
    switch (action) {
      ...
      case "add_lp":
        ...
        if (token0Id === token1Id) {
          ...
          toastStore.error(
            "Cannot create LP link: Cannot pair token with itself.",
          );
          shouldCloseDropdown = true; // Close on error
          break;
        }
  ...
  >
    <svelte:fragment slot="actions">
      ...
      >
        <Shuffle
          size={12}
          class={isSyncing ? "text-kong-primary animate-glow" : ""}
        />
        <span class={isSyncing ? "text-kong-primary" : ""}
          >{isSyncing ? "Syncing..." : "Sync"}</span
        >
      </button>
        ...
        <div class="flex gap-4 text-xs">
          <div class="flex flex-col">
            ...
            <span class="text-kong-text-primary"
              >${poolStats.total_volume_24h.toLocaleString()}</span
            >
          </div>
          <div class="flex flex-col">
            <span class="text-kong-text-secondary/60">TVL</span>
            <span class="text-kong-text-primary"
              >${poolStats.total_tvl.toLocaleString()}</span
            >
          </div>
          <div class="flex flex-col">
            <span class="text-kong-text-secondary/60">24h Fees</span>
            <span class="text-kong-text-primary"
              >${poolStats.total_fees_24h.toLocaleString()}</span
            >
          </div>
        </div>
      {/if}
