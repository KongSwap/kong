<script lang="ts">
  ...
  let timeString = $derived(
    new Date(Number(pending.created_at / BigInt(1000000))).toLocaleTimeString(
      [],
      {
        hour: "2-digit",
        minute: "2-digit",
...
<div class="flex flex-col items-end" transition:fade={{ duration: 150 }}>
  <div class="flex items-end gap-1.5 max-w-[85%]">
    <div
      class="bg-kong-primary/10 text-white rounded-t-md rounded-bl-md rounded-br-sm px-3 py-2 opacity-70 relative"
    >
      ...
      <div
        class="flex items-center gap-1 text-xs text-kong-text-secondary mt-0.5"
      >
        <span class="opacity-70">Sending</span>
        <span
          class="w-2 h-2 rounded-full border border-kong-text-secondary/40 border-t-transparent animate-spin"
        ></span>
