...
<div class="p-2.5 bg-kong-bg-primary relative">
    ...
    <div class="flex gap-2 relative">
      ...
      {#if messageInput.trim()}
        ...
        >
          ...
          >
            <path
              d="M10 14 21 3m0 0-6.5 18a.55.55 0 0 1-1 0L10 14l-7-3.5a.55.55 0 0 1 0-1L21 3"
            />
          </svg>
        </button>
      {/if}
      ...
      {#if showEmojiPicker}
        ...
        >
          <emoji-picker onemoji-click={handleEmojiSelect} class="light"
          ></emoji-picker>
        </div>
      {/if}
    </div>
  {:else}
    <div
      class="text-center text-sm text-kong-text-secondary py-2.5 border border-dashed border-kong-border/30 rounded-md"
    >
