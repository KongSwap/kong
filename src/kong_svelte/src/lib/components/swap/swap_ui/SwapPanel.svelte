<script lang="ts">
  ...
  const handleMaxClick = () => {
    if (!disabled && title === "You Pay" && tokenInfo) {
      try {
        const maxAmount = SwapService.calculateMaxAmount(
          {
            canister_id: tokenInfo.canister_id,
            fee: tokenInfo.fee,
            icrc1: tokenInfo.icrc1,
            icrc2: tokenInfo.icrc2,
          },
          formattedBalance,
          decimals,
          isIcrc1,
        ...
        onAmountChange(
          new CustomEvent("input", { detail: { value: formattedMaxAmount } }),
        );
        animatedAmount.set(maxAmount.toNumber(), {
          duration: 400,
          easing: cubicOut,
        });
      } catch (error) {
      ...
      <div class="amount-container">
        <div class="input-wrapper">
          <input
            ...
            type="text"
            class="amount-input {isOverBalance && title === 'You Pay'
              ? 'error'
              : ''}"
            value={displayAmount}
            oninput={handleInput}
