<script lang="ts">
  ...
  // Calculated values
  $: token0Value = token0?.metrics?.price ? (Number(amount0) * Number(token0.metrics.price)).toFixed(2) : "0";
  $: token1Value = token1?.metrics?.price ? (Number(amount1) * Number(token1.metrics.price)).toFixed(2) : "0";
  $: totalValue = (Number(token0Value) + Number(token1Value)).toFixed(2);
  $: exchangeRate = !isCreatingPool && amount0 && amount1 ? 
  ...
  async function handleConfirm() {
    ...
    try {
      if (isCreatingPool) {
        ...
        toastStore.info(`Adding liquidity to ${token0.symbol}/${token1.symbol} pool...`);
