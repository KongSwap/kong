<script lang="ts">
  ...
  // For animation of new bets
  export let newBetIds: Set<string> = new Set();

  // Helper function to safely get bet data regardless of structure
  ...
  // Cache bets only when new bets are available, so that if the API returns an empty
  // array temporarily, we still show the previously rendered bets.
  let cachedBets: any[] = [];
  $: if (bets.length > 0) {
    cachedBets = bets;
  }

  // In the <script> section, add this reactive declaration after exporting newBetIds
  $: newBetIdsArray = Array.from(newBetIds);
</script>
...
>
  <div class="sticky top-4">
    <h2 class="text-sm uppercase font-medium">
      {title}
    </h2>
    <div class="max-h-[{maxHeight}] overflow-y-auto scrollbar-thin relative">
      {#if cachedBets.length > 0}
        {#each cachedBets as bet}
          {@const betData = getBetData(bet)}
          {#key betData.timestamp + '-' + betData.outcome_index + '-' + (newBetIdsArray.includes(`${betData.timestamp}-${betData.outcome_index}`) ? 'new' : 'old')}
            <div
              class="flex flex-col py-3 border-b border-kong-border/50 last:border-0 {newBetIdsArray.includes(`${betData.timestamp}-${betData.outcome_index}`) ? 'flash-new-bet' : ''} hover:bg-kong-bg-dark/30 transition-colors group"
            >
        ...
        {#if loading}
          <div class="absolute top-2 right-2" transition:fade>
            <span class="px-2 py-1 bg-kong-bg-dark bg-opacity-75 text-xs text-white rounded">Loading...</span>
          </div>
        {/if}
      {:else}
        {#if loading}
