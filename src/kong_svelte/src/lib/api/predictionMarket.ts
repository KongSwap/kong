...
export async function placeBet(token: FE.Token, marketId: number, outcomeIndex: number, amount: string) {
    try {
        console.log("Received in placeBet:", {
            amount,
            marketId,
            outcomeIndex,
            type: typeof amount
        ...
        console.log("Converting to BigInt:", {
            originalAmount: amount,
            bigIntAmount,
            isSameAsOriginal: bigIntAmount.toString() === amount
        });
...
export async function resolveMarketViaOracle(marketId: number, outcomeIndices: number[], signature: Uint8Array) {
    const actor = createAnonymousActorHelper(PREDICTION_MARKETS_CANISTER_ID, canisterIDLs.prediction_markets_backend);
    const result = await actor.resolve_via_oracle(marketId, outcomeIndices, Array.from(signature));
    return result;
}

export async function getAllBets(fromIndex: number = 0, toIndex: number = 10) {
    const actor = createAnonymousActorHelper(PREDICTION_MARKETS_CANISTER_ID, canisterIDLs.prediction_markets_backend);
    const bets = await actor.get_all_bets(BigInt(fromIndex), BigInt(toIndex), true);
    return bets;
}
