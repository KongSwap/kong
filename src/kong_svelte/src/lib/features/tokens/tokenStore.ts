...
function createTokenStore() {
  ...
  const loadTokens = async (forceRefresh = false) => {
    ...
    store.update(s => ({ 
      ...s, 
      isLoading: true,
      tokens: [] // Clear existing tokens when starting a fresh load
    }));

    try {
      const baseTokens = await TokenService.fetchTokens();
      const icTokens = baseTokens.filter(token => 'IC' in token);
      console.log("icTokens", icTokens);

      // Process each token one by one
      for (const token of icTokens) {
        try {
          const enrichedToken = await TokenService.enrichTokenWithMetadata(token);

          // Update the store incrementally with each new token
          store.update(s => ({
            ...s,
            tokens: [...s.tokens, enrichedToken],
            lastTokensFetch: Date.now()
          }));

        } catch (error) {
          console.error('Error enriching token:', token, error);
        }
      }

      // Set loading to false after all tokens are processed
      store.update(s => ({
        ...s,
        isLoading: false
      }));

      return get(store).tokens;
    } catch (error) {
  ...
  const getBalance = (canisterId: string) => {
    const currentState = getCurrentState();
  ...
  const reloadTokensAndBalances = async () => {
    try {
      await loadTokens(true); // Force refresh
      await loadBalances();
    } catch (error) {
...
export const portfolioValue: Readable<string> = derived(
  ...
  ([$balances, $tokens]) => {
    return calculatePortfolioValue($balances, $tokens)
  }
);
