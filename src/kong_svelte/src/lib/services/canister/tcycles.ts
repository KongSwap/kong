...
export const idlFactory = (args: { IDL: any }) => {
  const { IDL } = args;
  const Uint8Array = IDL.Vec(IDL.Nat8);
  const Account = IDL.Record({
    'owner': IDL.Principal,
    'subaccount': IDL.Opt(Uint8Array),
  });
  ...
  const CreateCanisterArgs = IDL.Record({
    'from_subaccount': IDL.Opt(Uint8Array),
  ...
  const CreateCanisterFromArgs = IDL.Record({
    'spender_subaccount': IDL.Opt(Uint8Array),
    'from': Account,
    'created_at_time': IDL.Opt(IDL.Nat64),
    'amount': IDL.Nat,
    'creation_args': IDL.Opt(CmcCreateCanisterArgs),
  ...
  const GetBlocksResult = IDL.Record({
    ...
    'archived_blocks': IDL.Vec(
      IDL.Record({
        'args': GetBlocksArgs,
        'callback': IDL.Tuple(IDL.Principal, IDL.Text),
      })
    ),
  });
  ...
  return IDL.Service({
    ...
    'create_canister_from': IDL.Func(
      ...
      [IDL.Variant({ 'Ok': CreateCanisterSuccess, 'Err': CreateCanisterFromError })],
      [],
    ),
...
// Export the types needed for initialization
export const init = (args: { IDL: any }) => {
  const { IDL } = args;
  return [];
}; 