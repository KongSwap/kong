...
/// Data structure for the reply of the `add_liquidity` function.
...
impl AddLiquidityReply {
    ...
    pub fn new_with_tx_id(tx_id: u64, add_liquidity_tx: &AddLiquidityTx) -> Self {
        let (symbol, chain_0, symbol_0, chain_1, symbol_1) = pool_map::get_by_pool_id(add_liquidity_tx.pool_id)
            .map_or_else(
                || {
                    (
