...
use crate::canister::id::{caller_account_id, caller_id};
...
// verify that the block_id is a transfer from caller to backend
pub async fn verify_transfer(token: &StableToken, block_id: &Nat, amount: &Nat, ts: u64) -> Result<(), String> {
                            ...
                            match block.transaction.operation {
                                Some(operation) => match operation {
                                    Operation::Transfer {
                                        ...
                                        if block.transaction.created_at_time.timestamp_nanos < ts {
