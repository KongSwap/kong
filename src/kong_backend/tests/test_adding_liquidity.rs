// src/kong_backend/tests/test_adding_liquidity.rs
//
// Tests for adding liquidity to a pool
...
#[derive(CandidType, Deserialize, Debug)]
struct AddLiquidityReply {
    pub request_id: u64,
}

/// Test for adding liquidity to an existing pool
#[test]
fn test_adding_liquidity() -> Result<()> {
    println!("Setting up test environment for adding liquidity test...");
    ...
    let user_a = Principal::from_slice(&[1, 2, 3, 4, 5]); // Initial pool creator
    ...
    println!("User A (pool creator) principal: {}", user_a.to_text());
    ...
    // Mint ICP to User A (pool creator)
    ...
    // Mint ICP to User B (liquidity provider)
    let icp_amount_b = Nat::from(500_000_000_000u64); // 5,000 ICP
    ...
    let ckusdt_amount_b = Nat::from(2_500_000_000_000u64); // 25,000 ckUSDT
    ...
    // Step 5: User A creates the initial pool
    println!("STEP 5: User A creating initial pool...");
    ...
    // Step 6: User B adds liquidity to the pool
    println!("STEP 6: User B adding liquidity to the pool...");
    
    // User B liquidity amounts
    let user_b_icp_liquidity = Nat::from(50_000_000_000u64); // 500 ICP
    let user_b_ckusdt_liquidity = Nat::from(5_000_000_000_000u64); // 50,000 ckUSDT
    
    // User B transfers ICP to Kong for liquidity
    let current_time_u64 = ic.get_time().as_nanos_since_unix_epoch();
    println!("Current IC time (ns): {}", current_time_u64);
    ...
    let transfer_args_icp = TransferArg {
        ...
        to: kong_account.clone(),
        amount: user_b_icp_liquidity.clone(),
        fee: None,
        memo: None,
        created_at_time: Some(current_time_u64),
    };
    ...
    let args_icp = encode_one(transfer_args_icp)?;
    println!("User B transferring ICP to Kong for liquidity");
    ...
    println!("User B transferred ICP to Kong for liquidity, tx_id: {:?}", icp_tx_id);
    
    // Multiple ticks to ensure the transaction is finalized
    ic.tick();
    ensure_processed(&ic);
    
    // User B transfers ckUSDT to Kong for liquidity
    let current_time_u64 = ic.get_time().as_nanos_since_unix_epoch();
    println!("Current IC time for ckUSDT transfer (ns): {}", current_time_u64);
    ...
    let transfer_args_ckusdt = TransferArg {
        from_subaccount: user_b_account.subaccount,
        to: kong_account.clone(),
        amount: user_b_ckusdt_liquidity.clone(),
        fee: None,
        memo: None,
        created_at_time: Some(current_time_u64),
    };
    ...
    let args_ckusdt = encode_one(transfer_args_ckusdt)?;
    println!("User B transferring ckUSDT to Kong for liquidity");
    ...
    println!("User B transferred ckUSDT to Kong for liquidity, tx_id: {:?}", ckusdt_tx_id);
    ...
    // User B adds liquidity using the transaction IDs from the transfers
    let add_liquidity_args = CanisterAddLiquidityArgs {
        token_0: format!("IC.{}", icp_ledger.to_string()),
        amount_0: user_b_icp_liquidity.clone(),
        tx_id_0: Some(icp_tx_id),
        token_1: format!("IC.{}", ckusdt_ledger.to_string()),
        amount_1: user_b_ckusdt_liquidity.clone(),
        tx_id_1: Some(ckusdt_tx_id),
    };

    let args = encode_one(add_liquidity_args)?;
    println!("User B sending add_liquidity request");
    
    // Advance time to help with transaction visibility
    ic.advance_time(std::time::Duration::from_secs(2));
    ic.tick();
    ensure_processed(&ic);
    
    // Call add_liquidity as User B to match the transfer sender
    let result = pic_update(&ic, kong_backend, user_b, "add_liquidity", args)?;
    ...
    match add_liquidity_result {
        AddLiquidityResult::Ok(reply) => {
            println!("User B added liquidity successfully, request ID: {}", reply.request_id);
        }
        AddLiquidityResult::Err(e) => panic!("Failed to add liquidity: {}", e),
    }

    ensure_processed(&ic);

    // Step 7: Query and verify pool state after adding liquidity
    println!("STEP 7: Verifying pool state after adding liquidity...");
    let get_pool_args = encode_one(None as Option<String>)?; // Get all pools
    let pool_result = pic_query(&ic, kong_backend, Principal::anonymous(), "pools", get_pool_args)?;
    let pool_result: PoolsResult = decode_one(&pool_result)
        .map_err(|e| anyhow::anyhow!("Failed to decode pools response: {}", e))?;
    ...
    let updated_pool = pools.into_iter()
        .find(|p| p.pool_id == pool_id)
        .expect("Pool not found");
    
    println!("Updated pool state:");
    println!("Token 0 (ICP) amount: {}", updated_pool.token_0_amount);
    println!("Token 1 (ckUSDT) amount: {}", updated_pool.token_1_amount);
    println!("LP token total supply: {}", updated_pool.lp_token_total_supply);
    
    // Verify pool balance increased correctly
    let expected_icp_amount = initial_pool.token_0_amount.clone() + user_b_icp_liquidity.clone();
    let expected_ckusdt_amount = initial_pool.token_1_amount.clone() + user_b_ckusdt_liquidity.clone();
    
    assert!(updated_pool.token_0_amount >= expected_icp_amount, 
            "Pool ICP amount didn't increase correctly");
    assert!(updated_pool.token_1_amount >= expected_ckusdt_amount, 
            "Pool ckUSDT amount didn't increase correctly");
    assert!(updated_pool.lp_token_total_supply > initial_pool.lp_token_total_supply,
            "LP token supply didn't increase after adding liquidity");
    
    println!("Adding liquidity test completed successfully!");
    Ok(())
}