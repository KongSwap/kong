...
use crate::common::canister::{create_canister, create_canister_at_id}; // Import necessary helper
...
pub fn create_kong_backend_at_id(
    ic: &PocketIc,
    ...
    let kong_canister_id = create_canister_at_id(ic, specified_id, controller)?;

    // 2. Read the Wasm module
    let wasm_module =
        fs::read(KONG_BACKEND_WASM).map_err(|e| format!("Failed to read Kong Wasm: {}", e))?;

    // 3. Encode the init arguments (currently unit `()`)
    let args = encode_one(()).map_err(|e| format!("Failed to encode Kong init args: {}", e))?;
