...
impl TestSetup {
    ...
    pub fn execute_add_liquidity(
        ...
        tx_id_a: Option<Nat>,
        tx_id_b: Option<Nat>,
    ) -> Result<Result<AddLiquidityReply, String>> {
        let (token_a_amount, token_b_amount) = self.calculate_amounts(config);
        ...
        let add_liquidity_payload = encode_one(&add_liquidity_args).expect("Failed to encode add_liquidity_args");

        let add_liquidity_response_bytes = self
            .ic
            .update_call(self.kong_backend, self.user_principal, "add_liquidity", add_liquidity_payload)
            .map_err(|e| anyhow::anyhow!("Failed to call add_liquidity: {:?}", e))?;

        let add_liquidity_result = decode_one::<Result<AddLiquidityReply, String>>(&add_liquidity_response_bytes)
            .expect("Failed to decode add_liquidity response");

        Ok(add_liquidity_result)
    }

    pub fn verify_balances(&self, config: &PoolTestConfig, add_liquidity_succeeded: bool) -> Result<()> {
        let (token_a_amount, token_b_amount) = self.calculate_amounts(config);
        ...
        if add_liquidity_succeeded {
            // If successful, Kong should have the full amounts
            assert_eq!(
                kong_balance_a, token_a_amount,
                "Kong backend balance for Token A incorrect after add_liquidity"
            );

            assert_eq!(
                kong_balance_b, token_b_amount,
                "Kong backend balance for Token B incorrect after add_liquidity"
            );
            ...
            assert!(
                ...
                "User balance for Token A is unreasonably high after successful add_liquidity"
            );

            assert!(
                user_balance_b <= _total_mint_b,
                "User balance for Token B is unreasonably high after successful add_liquidity"
            );
        } else {
            // If failed, Kong balances should be at most a few fees worth
            assert!(
                kong_balance_a <= self.token_a_fee.clone() * Nat::from(2u64),
                "Kong balance for Token A is unexpectedly high after failed add_liquidity: {}",
                kong_balance_a
            );

            assert!(
                kong_balance_b <= self.token_b_fee.clone() * Nat::from(2u64),
                "Kong balance for Token B is unexpectedly high after failed add_liquidity: {}",
                kong_balance_b
            );
            ...
            if !(config.token_a_amount_factor < 1.0 && config.use_token_a_approval) {
                assert!(
                    user_balance_a > 0u64,
                    "User balance for Token A should not be zero after failed add_liquidity"
                );
            }
            ...
            if !(config.token_b_amount_factor < 1.0 && config.use_token_b_approval) {
                assert!(
                    user_balance_b > 0u64,
                    "User balance for Token B should not be zero after failed add_liquidity"
                );
            }
        }
...
fn setup_test_tokens(
    ...
    let token_a_ledger_id = Principal::from_text("zdzgz-siaaa-aaaar-qaiba-cai").expect("Invalid ksUSDT Principal ID");
    let token_b_ledger_id = Principal::from_text("nppha-riaaa-aaaal-ajf2q-cai").expect("Invalid ICP Principal ID");
...
pub fn run_pool_test(config: PoolTestConfig) -> Result<()> {
    ...
    if let ExpectedOutcome::OtherUserFailure { error_contains } = &config.expected_outcome {
        ...
        let add_liquidity_args = AddLiquidityArgs {
            ...
            tx_id_1: other_user_tx_id_b.map(TxId::BlockIndex),
            signature_0: None,
            signature_1: None, // lp_fee_bps: None,
        };

        let add_liquidity_payload = encode_one(&add_liquidity_args).expect("Failed to encode add_liquidity_args");

        let add_liquidity_response_bytes = setup
            .ic
            .update_call(setup.kong_backend, setup.user_principal, "add_liquidity", add_liquidity_payload)
            .map_err(|e| anyhow::anyhow!("Failed to call add_liquidity with other user tx_id: {:?}", e))?;

        let result_with_other_tx = decode_one::<Result<AddLiquidityReply, String>>(&add_liquidity_response_bytes)
            .expect("Failed to decode add_liquidity response");

        assert!(
            result_with_other_tx.is_err(),
            "Expected failure when using other user's tx ID but got success"
        );
    ...
    // 4. Create and execute add_liquidity call
    let result = setup.execute_add_liquidity(&config, tx_id_a, tx_id_b)?;
...
#[test]
fn test_add_liquidity_using_approvals() {
    let config = PoolTestConfig::standard().with_token_a_approval().with_token_b_approval();

    run_pool_test(config).unwrap();
}

#[test]
fn test_add_liquidity_using_transfers() {
...
#[test]
fn test_add_liquidity_with_mixed_methods() {
    let config = PoolTestConfig::standard().with_token_a_approval();
...
#[test]
fn test_add_liquidity_insufficient_token_a_balance() {
    let config = PoolTestConfig::standard()
...
#[test]
fn test_add_liquidity_insufficient_token_b_balance() {
    let config = PoolTestConfig::standard()
        .with_token_a_approval()
        .with_token_b_approval()
...
#[test]
fn test_add_liquidity_insufficient_allowance() {
    let config = PoolTestConfig::standard()
        .with_token_a_approval()
        .with_token_b_approval()
...
#[test]
fn test_add_liquidity_with_custom_fee() {
    let config = PoolTestConfig::standard().with_lp_fee_bps(30); // 0.3% fee

    run_pool_test(config).unwrap();
}

#[test]
fn test_add_liquidity_with_other_user_tx_id() {
    ...
    let add_liquidity_args = AddLiquidityArgs {
        ...
        token_1: token_b_str,
        amount_1: token_b_amount,
        tx_id_1: Some(TxId::BlockIndex(tx_id_b)),
        signature_0: None,
        signature_1: None, // Using OTHER user's tx ID
                           // lp_fee_bps: None,
    };

    let add_liquidity_payload = encode_one(&add_liquidity_args).expect("Failed to encode add_liquidity_args");

    let add_liquidity_response_bytes = ic
        .update_call(kong_backend, user_principal, "add_liquidity", add_liquidity_payload)
        .expect("Failed to call add_liquidity");

    let add_liquidity_result =
        decode_one::<Result<AddLiquidityReply, String>>(&add_liquidity_response_bytes).expect("Failed to decode add_liquidity response");

    // Verify failure - should reject because the tx IDs belong to a different user
    assert!(
        add_liquidity_result.is_err(),
        "Expected failure when using other user's tx ID but got success"
    );

    // TODO: this is a hack to check if the error contains the expected text
    if let Err(err) = add_liquidity_result {
