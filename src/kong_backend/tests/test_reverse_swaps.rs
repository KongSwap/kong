// src/kong_backend/tests/test_reverse_swaps.rs
//
// Tests for performing reverse token swaps (token B to token A) using approve/transferFrom
...
/// Test for performing reverse token swaps (token B to token A) using approve/transferFrom
#[test]
fn test_reverse_swaps_with_approve() -> Result<()> {
    println!("Setting up test environment for reverse swap test...");
    ...
    // Mint ckUSDT to User B (trader)
    let ckusdt_amount_b = Nat::from(1_000_000_000_000u64); // 10,000 ckUSDT
    ...
    // Step 7: User B performs reverse swap (ckUSDT -> ICP) using approve/transferFrom
    println!("STEP 7: User B performing reverse swap (ckUSDT -> ICP) using approve...");
    
    // User B approves Kong to spend ckUSDT tokens
    ...
    let args = encode_one(approve_args)?;
    println!("User B approving ckUSDT for Kong backend");
    let result = pic_update(&ic, ckusdt_ledger, user_b, "icrc2_approve", args)?;
    let _: Nat = decode_one(&result)?;
    println!("User B approved ckUSDT tokens for Kong backend");
    ensure_processed(&ic);

    // Query the allowance to verify it was set correctly
    let allowance_args = encode_one((user_b_account.clone(), kong_account.clone()))?;
    let result = pic_query(&ic, ckusdt_ledger, Principal::anonymous(), "icrc2_allowance", allowance_args)?;
    let allowance: ApproveArgs = decode_one(&result)?;
    println!("Allowance set: {}", allowance.amount);

    // User B executes the swap with transferFrom
    let swap_args = SwapArgs {
        ...
        block_id: None, // Not needed for transferFrom
        use_transfer_from: Some(true), // Important: Use transferFrom
    };

    let args = encode_one(swap_args)?;
    println!("User B executing swap with transferFrom");
    let result = pic_update(&ic, kong_backend, user_b, "swap_transfer", args)?;
    let _: serde_bytes::ByteBuf = decode_one(&result)?;
    println!("User B's reverse swap completed successfully");
    ensure_processed(&ic);

    // Step 8: Verify balances after reverse swap
    println!("STEP 8: Verifying balances after reverse swap...");
    ...
    // Verify User B received ICP
    assert!(user_b_icp_balance_after > user_b_icp_balance_initial, 
            "User B's ICP balance should have increased after swap");
    
    // Verify User B spent ckUSDT
    assert!(user_b_ckusdt_balance_after < user_b_ckusdt_balance_initial, 
            "User B's ckUSDT balance should have decreased after swap");
    ...
    assert!(pool_after_swap.token_0_amount < initial_pool.token_0_amount, 
            "Pool ICP amount should have decreased after swap");
    assert!(pool_after_swap.token_1_amount > initial_pool.token_1_amount, 
            "Pool ckUSDT amount should have increased after swap");
    
    // Calculate and print swap amounts
    let icp_received = user_b_icp_balance_after.clone() - user_b_icp_balance_initial.clone();
    let ckusdt_used = user_b_ckusdt_balance_initial.clone() - user_b_ckusdt_balance_after.clone();
    
    println!("User B reverse swap summary:");
    println!("ckUSDT used: {}", ckusdt_used);
    println!("ICP received: {}", icp_received);
    println!("Effective exchange rate: {} ICP per ckUSDT", 
             icp_received.clone() * Nat::from(100_000_000u64) / ckusdt_used.clone());
    
    println!("Reverse swap test (using approve/transferFrom) completed successfully!");
    Ok(())
}