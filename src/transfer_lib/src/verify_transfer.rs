...
use kong_lib::helpers::nat_helpers::nat_to_u64;
...
const TRANSFER_EXPIRY_NANOSECS: u64 = 3_600_000_000_000;
...
pub async fn verify_transfer(token: &StableToken, dst_address: &Address, block_id: &Nat, amount: &Nat) -> Result<Nat, String> {
    let canister_address = match dst_address {
        Address::AccountId(_) => return Err("invalid acc_id dst_address".to_string()),
        Address::PrincipalId(account) => account,
        Address::SolanaAddress(_) => return Err("invalid sol dst_address".to_string()),
    };
    ...
    match token {
        StableToken::IC(ic_token) => {
            ...
            let min_valid_timestamp = ICNetwork::get_time() - TRANSFER_EXPIRY_NANOSECS;
            ...
            if token_address_with_chain == ICP_CANISTER_ID {
                return verify_transfer_with_query_blocks(token, block_id, amount, canister_id, min_valid_timestamp, canister_address)
                    .await;
            }
    ...
    match blocks_result {
        Ok(response) => {
            ...
            for block_envelope in blocks_data.blocks.iter() {
                ...
                let mut res_transfer_amount = Nat::default();
                if let Some(ICRC3Value::Map(tx)) = tx_map {
                    // Amount
                ...
                match tx_amount {
                    Some(transfer_amount) if transfer_amount == *amount => res_transfer_amount = transfer_amount,
                ...
                return Ok(res_transfer_amount); // success
            }
                ...
                match block.transaction.operation {
                    Some(operation) => match operation {
                        Operation::Transfer {
                            ...
                            return Ok(Nat::from(transfer_amount.e8s())); // success
                        }
                        Operation::Mint { .. } => (),
                        Operation::Burn { .. } => (),
                        Operation::Approve { .. } => (),
