...
use crate::{
    chains::chains::IC_CHAIN,
    ic::{icp::{ICP_ADDRESS, ICP_SYMBOL}, id},
...
pub fn get_address_from_param(token: &StableToken, address: &Option<String>) -> Result<Address, String> {
    match token {
        StableToken::LP(_) => return Err("Invalid lp address from param".to_string()),
        StableToken::IC(ictoken) => {
            if ictoken.is_icp()
            {
                match address {
                    Some(addr) => get_address(token, &addr),
                    None => Ok(Address::AccountId(id::caller_account_id())),
                }
            }
            else
            {
                match address {
                    Some(addr) => get_address(token, &addr),
                    None => Ok(Address::PrincipalId(id::caller_id())),
                }
            }
        ...
        StableToken::Solana(_) => {
            match address {
                Some(addr) => get_address(token, &addr),
                None => Err("Solana optional address is required".to_string()),
            }
        },
    }
...
pub fn get_address(token: &StableToken, address: &str) -> Result<Address, String> {
    ...
    match token {
        StableToken::LP(_) => return Err("Invalid lp address".to_string()),
        StableToken::IC(_) => {
            if regrex_princiapl_id.is_match(address) {
                ...
                if token.chain() == IC_CHAIN && token.symbol() == ICP_SYMBOL && token.address() == ICP_ADDRESS {
        ...
        StableToken::Solana(_) => {
            crate::solana::validation::validate_address(address).map_err(|e| format!("Invalid Solana address: {}", e))?;
            Ok(Address::SolanaAddress(address.to_string()))
        }
    }
}
