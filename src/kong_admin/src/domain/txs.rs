...
pub struct TokenCache {
    ...
    pool_tokens: HashMap<u32, (u32, u32)>,
}

impl TokenCache {
    pub fn new(tokens_map: &BTreeMap<u32, u8>, pools_map: &BTreeMap<u32, (u32, u32)>) -> Self {
        Self {
            decimals: tokens_map.iter().map(|(&k, &v)| (k, v)).collect(),
            pool_tokens: pools_map.iter().map(|(&k, &v)| (k, v)).collect(),
        }
    }
    ...
    #[inline]
    fn get_pool_tokens(&self, pool_id: u32) -> Result<(u32, u32), Box<dyn std::error::Error>> {
        self.pool_tokens
            .get(&pool_id)
            .copied()
            .ok_or_else(|| format!("pool_id={} not found", pool_id).into())
    }
}

pub struct PreparedStatements {
    insert_tx: Statement,
    insert_add_pool: Statement,
    insert_add_liquidity: Statement,
    insert_remove_liquidity: Statement,
    insert_swap: Statement,
    insert_swap_pool: Statement,
    insert_send: Statement,
}
    ...
    pub async fn new(client: &Client) -> Result<Self, Box<dyn std::error::Error>> {
        Ok(Self {
            ...
            insert_add_liquidity: client
                ...
                .await?,
            insert_remove_liquidity: client
                .prepare(
...
pub async fn update_txs_on_database(
    ...
    // Get a connection and prepare statements once
    let mut db_client = db_pool.get().await?;
    let cache = TokenCache::new(tokens_map, pools_map);
    let statements = PreparedStatements::new(&db_client).await?;
    ...
    const BATCH_SIZE: usize = 1000; // Smaller batch size for txs due to complexity
    let mut batch: Vec<StableTx> = Vec::with_capacity(BATCH_SIZE);
    let mut total_processed = 0;
    let start_time = std::time::Instant::now();

    for (file_num, file_path) in files {
        println!("Processing txs file {}: {:?}", file_num, file_path.file_name().unwrap());
        ...
        for (_, v) in txs_map.iter() {
            ...
            if batch.len() >= BATCH_SIZE {
                let batch_refs: Vec<&StableTx> = batch.iter().collect();
                process_tx_batch(&mut db_client, &cache, &statements, &batch_refs).await?;
                ...
                if total_processed % 5000 == 0 {
                    ...
                    println!("Progress: {} txs processed ({:.0} tx/sec)", total_processed, rate);
                }
            }
        }
    }
    
    // Process remaining txs
    if !batch.is_empty() {
        let batch_refs: Vec<&StableTx> = batch.iter().collect();
        process_tx_batch(&mut db_client, &cache, &statements, &batch_refs).await?;
        total_processed += batch.len();
    ...
    println!("All txs processed. Total: {} txs in {:.2}s ({:.0} tx/sec)",
...
/// Process a batch of transactions with a database transaction for better performance
async fn process_tx_batch(
    ...
    // Process all txs in the batch
    for tx in txs {
        if let Err(e) = insert_tx_on_database_transactional(tx, &transaction, cache, statements).await {
            // Rollback on error
            transaction.rollback().await?;
            return Err(format!("Failed to insert tx {}: {}", tx.tx_id(), e).into());
        }
    }
    ...
    println!("Processed batch of {} txs", txs.len());
    Ok(())
}
...
pub async fn insert_tx_on_database(
    ...
    match v {
        StableTx::AddPool(v) => {
            ...
            let (token_id_0, token_id_1) = pools_map.get(&v.pool_id)
                .ok_or(format!("pool_id={} not found in pools_map", v.pool_id))?;
...
pub async fn insert_tx_on_database_optimized(
    v: &StableTx,
    db_client: &Client,
    cache: &TokenCache,
    statements: &PreparedStatements,
    ...
    match v {
        StableTx::AddPool(v) => {
            ...
            let (token_id_0, token_id_1) = cache.get_pool_tokens(v.pool_id)?;
            let decimals_0 = cache.get_decimals(token_id_0)?;
            let decimals_1 = cache.get_decimals(token_id_1)?;
        ...
        StableTx::AddLiquidity(v) => {
            ...
            db_client
                .execute(
                    &statements.insert_add_liquidity,
                    &[
                        &tx_id,
                        &pool_id,
                        &request_id,
        ...
        StableTx::RemoveLiquidity(v) => {
            ...
            db_client
                .execute(
                    &statements.insert_remove_liquidity,
                    &[
                        &tx_id,
                        &pool_id,
                        &request_id,
        ...
        StableTx::Swap(v) => {
            ...
            let pay_token_id = v.pay_token_id as i32;
            let pay_decimal = cache.get_decimals(v.pay_token_id)?;
            let pay_amount = round_f64(
                v.pay_amount.0.to_f64().unwrap() / 10_u64.pow(pay_decimal as u32) as f64,
                pay_decimal,
            );
            let receive_token_id = v.receive_token_id as i32;
            let receive_decimal = cache.get_decimals(v.receive_token_id)?;
            let receive_amount = round_f64(
                v.receive_amount.0.to_f64().unwrap() / 10_u64.pow(receive_decimal as u32) as f64,
                receive_decimal,
            );
            ...
            for swap in v.txs.iter() {
                let pool_id = swap.pool_id as i32;
                let pay_token_id = swap.pay_token_id as i32;
                let pay_decimal = cache.get_decimals(swap.pay_token_id)?;
                let pay_amount = round_f64(
                    swap.pay_amount.0.to_f64().unwrap() / 10_u64.pow(pay_decimal as u32) as f64,
                    pay_decimal,
                );
                let receive_token_id = swap.receive_token_id as i32;
                let receive_decimal = cache.get_decimals(swap.receive_token_id)?;
                let receive_amount = round_f64(
                    swap.receive_amount.0.to_f64().unwrap() / 10_u64.pow(receive_decimal as u32) as f64,
                    receive_decimal,
                );
    ...
    println!("tx_id={} saved (optimized)", v.tx_id());

    Ok(())
}

/// Insert transaction using a database transaction for better performance and atomicity
pub async fn insert_tx_on_database_transactional(
    v: &StableTx,
