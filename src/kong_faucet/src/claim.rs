...
use crate::ic::constants::{CKBTC, CKBTC_LEDGER, CKUSDT, CKUSDT_LEDGER, ICP, ICP_LEDGER};
...
// Commented out tokens that don't exist in local deployment
...
pub async fn claim() -> Result<String, String> {
    ...
    match join!(
        ...
        // Removed non-existent tokens from the join! macro
    ) {
        (Ok(_), Ok(_), Ok(_)) => {
