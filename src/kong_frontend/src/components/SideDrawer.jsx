...
const SideDrawer = ({ isOpen, onClose, children, headTitle, customHead }) => {
  ...
  return (
    <section className={`side-drawer ${isOpen ? 'open' : ''} ${isAnimating ? 'opening' : ''}`}>
      {isOpen && <div className="side-drawer-overlay" onClick={onClose} />}
      <div className="side-drawer-container" onClick={(e) => e.stopPropagation()}>
        <div className="side-drawer-close" onClick={onClose}>
          <svg
            className="side-drawer-close-svg"
            width="24"
            height="24"
            viewBox="0 0 24 24"
            fill="none"
